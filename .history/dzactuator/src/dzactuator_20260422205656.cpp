...
//          ＼二つ
  ...
  // ── buchan：激光与镜头物理偏差补偿（移植自冠军版，用置信度近似框大小）──
  ...
  // ── 比例超前补偿（替换原固定 OFFSET_X=23，改为与速度成比例）─────────────
  // 冠军版思路：offset ∝ v，靶子快时超前量大，靶子慢时超前量小
  static constexpr double K_LEAD = 2.5;  // 可调
  int offset_x = static_cast<int>(K_LEAD * v_x);
  int offset_y = static_cast<int>(K_LEAD * v_y);

  // ── 位置指令（移植冠军版 displace_coefficient，替换原固定 /2）────────────
  // 冠军版默认 displace_coefficientdouble = 1.1
  static constexpr double DISPLACE_COEFF = 1.1;  // 可调
  moveBaseControl.Position_0 = curYuntai_feedback_data.Position_0
                               + static_cast<int>(DISPLACE_COEFF * (cur_y + buchan))
                               + offset_y;
  moveBaseControl.Position_1 = curYuntai_feedback_data.Position_1
                               + static_cast<int>(DISPLACE_COEFF * cur_x)
                               + offset_x;
  ...
  double speed_0 = std::abs(KP * (cur_y + buchan) * DISPLACE_COEFF + KV * (v_y * DISPLACE_COEFF));
  double speed_1 = std::abs(KP * cur_x * DISPLACE_COEFF + KV * (v_x * DISPLACE_COEFF));
   ...
   && (ros::Time::now() - last_shot_time).toSec() >= SHOT_INTERVAL)
  {
    ...
    ROS_INFO("SHOOT! offset_x=%d offset_y=%d buchan=%d threshold_x=%.1f threshold_y=%.1f",
