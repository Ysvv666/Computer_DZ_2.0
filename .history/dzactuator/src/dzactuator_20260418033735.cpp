...
void turn_on_robot::CaremaMontorControl()
{
  ...
  */
  //**************这边是云台移动策略第一阶段 停车点寻找物资***********//
   ...
   // 静态变量：只初始化一次，全程保留值
    ...
    if((ros::Time::now() - find_start).toSec() <0.5){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑).(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1900;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 3000;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
 ...
 //**************这边是云台移动策略第三阶段 “自由阶段”，不受停车点限制，如果没看到靶子才保持朝前状态***********//
  ...
  {
    if(current_road_point_index==1){
      moveBaseControl.Position_0 = 1850;//根据靶子高度自行决定
      moveBaseControl.Position_1 = 1000;//云台水平初始位置，根据实际情况调整
    }
    else if(current_road_point_index==2){
      moveBaseControl.Position_0 = 1900;//根据靶子高度自行决定
      moveBaseControl.Position_1 = 3000;//云台水平初始位置，根据实际情况调整
    }
    else if(current_road_point_index==3){
      moveBaseControl.Position_0 = 1850;//根据靶子高度自行决定
      moveBaseControl.Position_1 = 1000;//云台水平初始位置，根据实际情况调整
    }    
    else if(current_road_point_index==4){
      moveBaseControl.Position_0 = 1900;//根据靶子高度自行决定
      moveBaseControl.Position_1 = 1000;//云台水平初始位置，根据实际情况调整
    }    
    else if(current_road_point_index==5){
      moveBaseControl.Position_0 = 1850;//根据靶子高度自行决定
      moveBaseControl.Position_1 = 3000;//云台水平初始位置，根据实际情况调整
    }    
    else if(current_road_point_index==8){
      moveBaseControl.Position_0 = 1900;//根据靶子高度自行决定
      moveBaseControl.Position_1 = 1000;//云台水平初始位置，根据实际情况调整
    }
