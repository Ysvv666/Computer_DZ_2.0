...
{  
 ...
 //*********************识别到靶子**********************//
    ...
    if(current_id == 116 || current_id == 117 || current_id == 118)//打靶点才打靶
...
//          ＼二つ         
  ...
  // 速度预测：用相邻两帧像素偏移差估算靶子运动速度，超前 LEAD 帧补偿延迟
  static int prev_offset_x = 0;
  static int prev_offset_y = 0;
  static constexpr float LEAD = 1.0f; // 预测帧数，越大越激进，可调

  int cur_x = temp_msg.data[0];
  int cur_y = temp_msg.data[1];
  int vel_x = cur_x - prev_offset_x; // x 方向像素速度（单帧）
  int vel_y = cur_y - prev_offset_y;
  prev_offset_x = cur_x;
  prev_offset_y = cur_y;

  // 预测偏移 = 当前偏移 + 速度 * 预测帧数
  int pred_x = cur_x + static_cast<int>(vel_x * LEAD);
  int pred_y = cur_y + static_cast<int>(vel_y * LEAD);

  moveBaseControl.Position_0 = curYuntai_feedback_data.Position_0 + (pred_y / 2);//云台Y轴位置（含速度超前）
  moveBaseControl.Position_1 = curYuntai_feedback_data.Position_1 + (pred_x / 2);//云台X轴位置（含速度超前）
  ...
  if(abs(temp_msg.data[1]) < 10 && abs(temp_msg.data[0]) <10 ) {//如果目标在视野中心Fire，并且急停（不急停怎么打FPS??）！！！  
