...
int After_ZhuanYunTai_flag = 0;   // 全局变量，默认0
...
void PublishPtzPose(ros::NodeHandle &nh, const curYuntai_feedback &feedback)
{
  ...
  geometry_msgs::Twist ptz_msg;
  ptz_msg.linear.x = feedback.Position_1;
  ptz_msg.linear.y = feedback.Position_0;
  ptz_msg.angular.x = feedback.Speed_1;
  ptz_msg.angular.y = feedback.Speed_0;
  pub_ptz_pose.publish(ptz_msg);
}

void PublishCameraTf(const curYuntai_feedback &feedback)
{
  static tf2_ros::StaticTransformBroadcaster tf_broadcaster;

  const double x_angle = ((2047 - feedback.Position_1) / 512.0) * (M_PI / 4);

  geometry_msgs::TransformStamped ts;
  ts.header.stamp = ros::Time::now();
  ts.header.frame_id = "base_link";
  ts.child_frame_id = "camera_joint";
  ts.transform.translation.x = 0.073;
  ts.transform.translation.y = 0.0;
  ts.transform.translation.z = 0.193;

  tf2::Quaternion qtn;
  qtn.setRPY(0, 0, x_angle);
  ...
  ts2.header.frame_id = "camera_joint";
  ts2.child_frame_id = "camera_link";
  ts2.transform.translation.x = 0.074;
  ts2.transform.translation.y = 0.0;
  ts2.transform.translation.z = 0.0;

  tf2::Quaternion qtn2;
  qtn2.setRPY(0, 0, 0);
  ts2.transform.rotation.x = qtn2.getX();
  ts2.transform.rotation.y = qtn2.getY();
  ts2.transform.rotation.z = qtn2.getZ();
  ts2.transform.rotation.w = qtn2.getW();

  tf_broadcaster.sendTransform(ts);
  tf_broadcaster.sendTransform(ts2);
}
...
turn_on_robot::turn_on_robot() : Power_voltage(0)
{
  ...
  moveBaseControl.Speed_0 = 10000;
  moveBaseControl.Speed_1 = 10000;
  ...
  sub_monter_control = n.subscribe("/ptz_control", 1, &turn_on_robot::callback_monter_control, this);     //云台控制回调（冠军版话题名）
  sub_stop_point_singal = n.subscribe("/move_base/stop_signal",1,&turn_on_robot::callback_stop_point_signal,this);              //停车状态回调

    // subscribe to published road points index (which stop point we are at)
    sub_road_points_index = n.subscribe("/road_points_index", 1, &turn_on_robot::callback_road_points_index, this);
  ...
  move_hit_goal_sub = n.subscribe("/move_hit_goal_flag", 1, &turn_on_robot::moveHitGoalCallback, this);
...
void turn_on_robot::Control()
{
  ...
  ros::Rate rate(30);//发送数据包的周期，单位毫秒
  while (ros::ok())
  {
...
{  
 ...
 //*********************识别到靶子**********************//
    ...
    {
            find_wuzi_flag = 0;//wuzi是物资的意思,找到物资标志位为false
            // ROS_INFO("find_wuzi_flag = %d", find_wuzi_flag);
            std_msgs::UInt8 fu;
            fu.data = static_cast<uint8_t>(find_wuzi_flag ? 1 : 0);
            pub_find_wuzi_flag.publish(fu);
...
{
  // ===== 冠军版移植：Position_1=linear.x（水平），Position_0=linear.y（垂直），速度按消息值赋 =====
  moveBaseControl.Position_1 = msg->linear.x;  // 水平轴（冠军版顺序）
  moveBaseControl.Position_0 = msg->linear.y;  // 垂直轴（冠军版顺序）

  if (msg->angular.y > 0)
    moveBaseControl.Speed_0 = msg->angular.y;
  else
    moveBaseControl.Speed_0 = 1000;

  if (msg->angular.x > 0)
    moveBaseControl.Speed_1 = msg->angular.x;
  else
...
{
  move_hit_goal_flag_local = static_cast<uint8_t>(msg->data);
}
void turn_on_robot::callback_road_points_index(const std_msgs::Int32::ConstPtr &msg)
{
...
{
  /*ysvv：current_road_point_index—>物资点x
  0->物资点1
  1->物资点2
  2->物资点3
  3->物资点4
  ...
  17->终点
  */
  //**************这边是云台移动策略第一阶段 停车点寻找物资***********//
   if(stop_point_signal_msg == 1 &&  current_road_point_index== 0){//************************物资点1
    // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！

    if(find_start_flag == 0)
    {
        find_start = ros::Time::now(); // 记录开始时间
        find_start_flag = 1;// 设置标志位，确保每次进入该if判断，只记录一次开始时间
    }

    if((ros::Time::now() - find_start).toSec() <0.8){//0.8秒内直接锁定，如果该时间段没锁定到就进入巡航
      moveBaseControl.Position_0 = 1920;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 1100;//云台上移部分，固定仰角，根据物资高度自行决定
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==1){//********************物资点2
     // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==2){//********************物资点3
    // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==3){//********************物资点4
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.3){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1900;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 2850;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
      //After_ZhuanYunTai_flag=1;
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==4){//********************物资点5
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.3){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1880;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 1000;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==6){//********************物资点6
    // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==5){//********************物资点7
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.3){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1940;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 3000;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==7){//********************物资点8
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.8){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1950;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 3000;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==8){//********************物资点9
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==9){//********************物资点10
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.3){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1900;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 970;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
      //After_ZhuanYunTai_flag=1;
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==11){//********************物资点11
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==10){//********************物资点12
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.3){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1900;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 1600;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
      //After_ZhuanYunTai_flag=1;
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==12){//********************物资点13
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.3){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1980;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 2800;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
      //After_ZhuanYunTai_flag=1;
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==13){//********************物资点14
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.3){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1990;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 2000;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
      //After_ZhuanYunTai_flag=1;
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==14){//********************物资点15
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
  ...
  else if(stop_point_signal_msg == 1 && current_road_point_index==15){//********************物资点16
   // 静态变量：只初始化一次，全程保留值
    static int find_start_flag = 0;
    static ros::Time find_start;  // 放在这里，作用域覆盖整个函数！
    ...
    if((ros::Time::now() - find_start).toSec() <0.8){//0.8秒内直接锁定，没锁定到就进入巡航,其实没啥用，够我锁定环节进入一次就足够啦.(其实是多余的，被我们淘汰的定位逻辑).(其实是多余的，被我们淘汰的定位逻辑)
      moveBaseControl.Position_0 = 1900;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Position_1 = 3070;//云台上移部分，固定仰角，根据物资高度自行决定
      moveBaseControl.Speed_0 = 10000;   // 云台Y轴速度
      moveBaseControl.Speed_1 = 10000;   // 云台X轴速度, 速度拉满，确保快速锁定物资
      //ros::Duration(1.2).sleep(); 
      //After_ZhuanYunTai_flag=1;
 ...
 //**************这边是云台移动策略第三阶段 “自由阶段”，不受停车点限制，如果没看到靶子才保持朝前状态***********//
...
//          ＼二つ
  ...
  // ── 位置误差低通滤波（步长=1时必须先平滑，否则检测噪声直接打到舵机）──────
  static double x_filtered = 0.0;
  static double y_filtered = 0.0;
  // α=0.4：0越平滑但跟移动靶有滞后，1=不滤波直接用原始值，可调
  static constexpr double ALPHA_POS = 0.05;
  x_filtered = (1.0 - ALPHA_POS) * x_filtered + ALPHA_POS * cur_x;
  y_filtered = 1.0  * y_filtered + ALPHA_POS * cur_y;
  ...
  double v_x_raw = cur_x - prev_x;  // 用原始值做差分
  double v_y_raw = cur_y - prev_y;
  prev_x = cur_x;
  prev_y = cur_y;
  // 低通滤波：α越小越平滑但响应越慢，可调
  v_x_filtered = 0.7 * v_x_filtered + 0.1 * v_x_raw;
  v_y_filtered = 0.7 * v_y_filtered + 0.001 * v_y_raw;
  ...
  static constexpr double K_LEAD = 1.2;  // 可调，若还抖动继续减小
  int offset_x = static_cast<int>(K_LEAD * v_x_filtered);

  // ── 位置指令（步长1，用滤波后的误差）────────────────────────────────────
  moveBaseControl.Position_0 = curYuntai_feedback_data.Position_0 + (cur_y_f + buchan);
  moveBaseControl.Position_1 = curYuntai_feedback_data.Position_1 + cur_x_f + offset_x;

  // ── 速度输出：P 项 + 速度前馈 ────────────────────────────────────────────
  static constexpr double KP = 500.0;  // 比例增益，可调
  static constexpr double KV = 150.0;  // 速度前馈增益，可调

  double speed_0 = std::abs(KP * (cur_y_f + buchan) + KV * v_y_filtered);
  double speed_1 = std::abs(KP * cur_x_f + KV * v_x_filtered);
  ...
  static constexpr double SHOT_INTERVAL = 0.05;  // 可调

  if (std::abs(cur_x_f) < fire_threshold_x
   && std::abs(cur_y_f + buchan) < fire_threshold_y
   && (ros::Time::now() - last_shot_time).toSec() >= SHOT_INTERVAL)
  {
    ...
    ROS_INFO("SHOOT! cur_x=%d cur_y=%d buchan=%d thr_x=%.1f thr_y=%.1f",
             cur_x, cur_y, buchan, fire_threshold_x, fire_threshold_y);
  }
}
...
double turn_on_robot::CaremaSpeedControl(int target_pose,int current_pose){
    ...
    double speed = kp * error +kd * filtered_derivative;
...
void turn_on_robot::sendCarInfoKernel()
{
  ...
  // 预留位 Reservel
  buf[21] = static_cast<int>(moveBaseControl.VoiceSwitch);
  // 校验和
  unsigned char sum = 0;
