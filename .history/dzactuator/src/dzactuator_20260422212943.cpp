...
//          ＼二つ
  ...
  v_x_filtered = 0.7 * v_x_filtered + 0.32 * v_x_raw;
  ...
  // ── 位置指令（步长1，全量误差直接叠加）──────────────────────────────────
  moveBaseControl.Position_0 = curYuntai_feedback_data.Position_0 + (cur_y + buchan);
  moveBaseControl.Position_1 = curYuntai_feedback_data.Position_1 + cur_x + offset_x;

  // ── 速度输出：P 项 + 速度前馈 ────────────────────────────────────────────
  static constexpr double KP = 500.0;  // 比例增益，可调
  static constexpr double KV = 150.0;  // 速度前馈增益，可调

  double speed_0 = std::abs(KP * (cur_y + buchan) + KV * v_y_filtered);
  double speed_1 = std::abs(KP * cur_x + KV * v_x_filtered);
