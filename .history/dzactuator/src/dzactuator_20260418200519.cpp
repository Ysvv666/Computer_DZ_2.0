...
//          ＼二つ         
  ...
  const int    PRED = 3;     // 预测帧数，越大超前越多，可调
  ...
  const double INNOV_GATE = 15.0; // 像素，可调
  ...
  // ── 线性插值速度（替代 PD，消除微分项在换向时的尖峰）────────────────────────
  int err0 = cur_y / 2;
  int err1 = (int)(pred_x / 1.5f);
  moveBaseControl.Speed_0 = gimbalLerpSpeed(err0);
  moveBaseControl.Speed_1 = gimbalLerpSpeed(err1);
...
double turn_on_robot::CaremaSpeedControl(int target_pose,int current_pose){
    ...
    static constexpr double kp =  600.0;              // 稳定的比例增益
